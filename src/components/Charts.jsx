...
export const PieChart = ({ data = [], currencySymbol = '$' }) => {
  ...
  return (
    <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-6 py-2">
      ...
      <div className="flex-1 w-full space-y-2">
        {slices.map((slice, idx) => (
          <div key={idx} className="flex items-center justify-between p-1.5 rounded-lg hover:bg-slate-50 transition-colors text-xs">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 rounded-full shadow-sm flex-shrink-0" style={{ backgroundColor: slice.color }}></span>
              <span className="font-semibold text-slate-700">{slice.name}</span>
            </div>
            <div className="text-right">
              <span className="font-bold text-slate-900 block">{formatVal(slice.value, currencySymbol)}</span>
              <span className="text-[10px] text-slate-400 block">{slice.percentage}%</span>
            </div>
          </div>
        ))}
      </div>
